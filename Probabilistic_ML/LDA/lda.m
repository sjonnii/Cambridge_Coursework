...
for iter = 1:20    % This can take a couple of minutes to run
  ...
  box_words(:,iter) = (sk + alpha)/(sum(sk) + K*alpha);
  probability = (swk+gamma)./sum(swk+gamma,1);
  entropy = nansum(probability.*log2(1./probability),1);
  entropy_matrix(:,iter) = entropy;
  lp = 0; nd = 0;
end
